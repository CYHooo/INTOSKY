...
@pytest.fixture()
def node():
    return rospy.init_node('deepsky_test', anonymous=True)
...
def test_blocks(node):
    rospy.wait_for_service('deepsky_blocks/run', timeout=5)
    rospy.wait_for_service('deepsky_blocks/stop', timeout=5)
    rospy.wait_for_service('deepsky_blocks/load', timeout=5)
    rospy.wait_for_service('deepsky_blocks/store', timeout=5)

    from std_msgs.msg import String
    from deepsky_blocks.srv import Run

    def wait_print():
        try:
            wait_print.result = rospy.wait_for_message('deepsky_blocks/print', String, timeout=5).data
    ...
    run = rospy.ServiceProxy('deepsky_blocks/run', Run)
...
def test_long_callback():
    from deepsky import long_callback
